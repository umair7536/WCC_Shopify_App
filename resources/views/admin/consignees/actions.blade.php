@if($consignee->active)
    @if(Gate::allows('consignees_inactive'))
        ...
        'route' => ['admin.consignees.inactive', $consignee->id])) !!}
...
@else
    @if(Gate::allows('consignees_active'))
        ...
        'method' => 'PATCH',
        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
        'route' => ['admin.consignees.active', $consignee->id])) !!}
        {!! Form::submit(trans('global.app_active'), array('class' => 'btn btn-xs btn-primary')) !!}
        {!! Form::close() !!}
    @endif
@endif
@if(Gate::allows('consignees_edit'))
    <a class="btn btn-xs btn-info" href="{{ route('admin.consignees.edit',[$consignee->id]) }}"
       data-target="#ajax_consignees" data-toggle="modal">@lang('global.app_edit')</a>
@endif
@if(Gate::allows('consignees_destroy'))
    {!! Form::open(array(
        ...
        'method' => 'DELETE',
        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
        'route' => ['admin.consignees.destroy', $consignee->id])) !!}
