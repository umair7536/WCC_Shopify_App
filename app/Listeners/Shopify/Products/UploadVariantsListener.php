...
class UploadVariantsListener implements ShouldQueue
{
    /**
     ...
     * Handle the event.
     *
     * @param  UploadVariantsFire  $event
     * @return void
     */
    public function handle(UploadVariantsFire $event)
    {
        ...
        if($event->account->id) {
            ...
            if($shop) {
                $total_records = ShopifyProductVariants::where([
                    'account_id' => $event->account->id,
                ])->count();
...
//            $a = '';
                ...
                if($total_records) {
                    ...
                    if($total_calls) {
                        ...
                        HeavyLifter::insert($shopify_jobs);
                    }
                }
            }

            echo 'Heavy Lift Queue dispatched for upload variants';
        }
